...
class Probe(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, backbone_output_dim: int, n_out_classes: int, attention_probe: bool):
        ...
        self.linear = nn.Linear(backbone_output_dim, n_out_classes)
    ...
    def save(self, path: str, epoch: int, optimizer: Optimizer, scheduler: torch.optim.lr_scheduler._LRScheduler):
        ...
        try:
            ...
            checkpoint = {
                ...
                'scheduler_state_dict': scheduler.state_dict() # <-- ADDED THIS
            }
            path_with_epoch = f"{path}_epoch_{epoch}.pt"
            torch.save(checkpoint, path_with_epoch)
            print(f"Successfully saved checkpoint to: {path_with_epoch}")
        except Exception as e:
            print(f"Error saving checkpoint to {path}: {e}")

    def load(self, path: str, optimizer: Optimizer = None, scheduler: torch.optim.lr_scheduler._LRScheduler = None, device: str = 'cpu'):
        ...
        try:
            ...
            if scheduler and 'scheduler_state_dict' in checkpoint:
                scheduler.load_state_dict(checkpoint['scheduler_state_dict']) # <-- ADDED THIS
